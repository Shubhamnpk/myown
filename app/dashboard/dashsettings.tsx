...
export function MoreInfoModal({
  ...
  const toggleWidget = (widgetId: string) => {
    ...
    onUpdateDashboard(visibleWidgetIds, colorScheme, widgetSize)
  }
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent, widgetId: string, action: "drag" | "resize") => {
      ...
      if (action === "drag") {
        ...
        setDragOffset({
          x: (e.clientX - rect.left) / scale - widget.position.x,
          y: (e.clientY - rect.top) / scale - widget.position.y,
        })
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (draggedWidget) {
        ...
        const maxX = (rect.width / scale) - widget.dimensions.width
        const maxY = (rect.height / scale) - widget.dimensions.height

        newX = Math.max(0, Math.min(newX, maxX))
        newY = Math.max(0, Math.min(newY, maxY))

        setWidgets((prev) =>
          prev.map((w) => (w.id === draggedWidget ? { ...w, position: { x: newX, y: newY } } : w)),
        )
      }
    ...
    [draggedWidget, resizingWidget, dragOffset, resizeStart, widgets, previewDevice],
  )
  ...
  return (
    <Dialog open={isOpen}>
      <style dangerouslySetInnerHTML={{ __html: `
        .custom-dialog-content > button.absolute {
          display: none;
        }
      ` }} />
      <DialogContent className="custom-dialog-content max-w-7xl w-full max-h-[95vh] overflow-y-auto bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <DialogTitle className="sr-only">Dashboard Studio</DialogTitle>
